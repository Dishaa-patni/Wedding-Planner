...
export default function DashboardSidebar({
  ...
  return (
    <aside
      className={`flex h-dvh w-[292px] shrink-0 flex-col overflow-hidden border-r border-[#F0DDD8] px-6 py-6 sm:px-8 ${className}`}
    >
      ...
      <nav className="mt-8 flex-1 space-y-1.5 overflow-hidden">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              onClick={onNavigate}
              className={`group flex h-10 items-center gap-4 rounded-2xl px-4 text-[15px] font-medium transition-all ${
                isActive
      ...
      <div className="mt-auto shrink-0">
        <div className="rounded-[22px] bg-white/58 p-3.5 shadow-[0_16px_48px_rgba(183,110,121,0.06)] ring-1 ring-[#F2DEDA] backdrop-blur-sm">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#FFD4CE] text-sm font-semibold text-white">
              AR
            </div>
        ...
        <Link
          href="/weddings"
          onClick={onNavigate}
          className="mt-4 inline-flex h-11 w-full items-center justify-center gap-2 rounded-full bg-[#D77474] px-4 text-sm font-semibold text-white shadow-[0_18px_40px_rgba(215,116,116,0.24)] transition-colors hover:bg-[#C96464]"
        >
